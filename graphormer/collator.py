...
class Batch():
    def __init__(self, idx, attn_bias, attn_edge_type, rel_pos, in_degree, out_degree, x, edge_input, y, smiles, pos):
    ...
    def to(self, device):
        ...
        self.pos = self.pos.to(device) # add
        return self
...
def collator(items, max_node=512, multi_hop_max_dist=20, rel_pos_max=20):
    ...
    items = [(item.idx, item.attn_bias, item.attn_edge_type, item.rel_pos, item.in_degree,
              item.out_degree, item.x, item.edge_input[:, :, :multi_hop_max_dist, :], item.y, item.smiles, item.pos) for item in items]
    idxs, attn_biases, attn_edge_types, rel_poses, in_degrees, out_degrees, xs, edge_inputs, ys, smiles, poss = zip(
        *items)
    ...
    smiles = smiles
    pos = torch.cat([pad_2d_unsqueeze(i, max_node_num) for i in poss]) # add padding
    return Batch(
